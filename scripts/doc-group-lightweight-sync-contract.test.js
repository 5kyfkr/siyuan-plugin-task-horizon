...
assert.match(syncRuntime, /if \(opt\.skipTaskReload === true\) \{[\s\S]*?applyFilters\(\);[\s\S]*?\} else \{[\s\S]*?await loadSelectedDocuments/, 'unchanged document scope must only refresh derived UI state');
