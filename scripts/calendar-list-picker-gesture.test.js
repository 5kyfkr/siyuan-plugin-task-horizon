...
vm.runInContext(source.slice(gestureStart, gestureEnd) + source.slice(actionStart, actionEnd)
    + ';this.gestures = {begin:beginMobileMonthSwipe, move:moveMobileMonthSwipe, finish:finishMobileMonthSwipe, target:mobileMonthSwipeTarget};this.action = performPrototypeListAction;', context);
function swipe(horizontal, vertical = 0, options = {}) {
    ...
    context.gestures.begin(event('pointerdown', 200, 100), 'pointer');
    context.gestures.move(event('pointermove', 200 + horizontal, 100 + vertical), 'pointer');
    context.gestures.finish(event('pointerup', 200 + horizontal, 100 + vertical), 'pointer', options.cancelled === true);
}
