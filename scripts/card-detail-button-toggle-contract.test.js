...
assert.match(openTaskDetail, /isQuickbarDetailOpen = String\(detailOpenOptions\.source \|\| ''\)\.trim\(\) === 'quickbar-detail-open'[\s\S]*useTaskDetailSheetMode = !isQuickbarDetailOpen[\s\S]*if \(!isQuickbarDetailOpen && activeRenderMode === 'kanban'/, 'document quickbar detail requests must bypass stale Sheet and kanban routes and use the standalone overlay');
