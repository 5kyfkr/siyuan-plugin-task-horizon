...
assert.match(checklist, /const taskDone = typeof __tmIsTaskClosedForDisplay === 'function'[\s\S]*?const doneCls = taskDone/,
...
for (const source of [timeline, legacyTimeline]) {
    assert.match(source, /const taskDone = typeof __tmIsTaskClosedForDisplay === 'function'[\s\S]*?const isDoneSubtask = taskDone/,
...
assert.match(whiteboard, /__tmRenderTaskCheckboxWrap\(tid, task, \{ checked: __tmIsTaskClosedForDisplay\(task\)/,
