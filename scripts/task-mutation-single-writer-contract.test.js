...
assert.match(kernel, /const laneID = text\(source\.laneID \|\| source\.laneId\) \|\| taskID;[\s\S]*return runTaskLane\(laneID,/);
...
assert.match(kernel, /async function moveTask\(input, options\)[\s\S]*return runTaskLane\(laneID,[\s\S]*await preserveTaskAttrsOnOwnBlockBeforeMove\(beforeTask\)/,
    'attribute preservation and the structural move must share the kernel task lane');
