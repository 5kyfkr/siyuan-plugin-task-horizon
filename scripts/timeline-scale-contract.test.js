...
assert.match(timelineMetaCallback, /const metaPatch = \{ milestone: val \? '1' : '' \};[\s\S]*__tmEnqueueTimelineMutation\(async \(\) =>[\s\S]*return await patchTask/, 'milestone callbacks must share the serial timeline mutation queue');
...
assert.match(gantt, /const shortClass = barWidth < 22 \? ' tm-gantt-bar--short' : '';[\s\S]*\$\{shortClass\}/, 'initial short task bars must expose the dedicated handle layout class');
assert.match(gantt, /const shortClass = width < 22 \? ' tm-gantt-bar--short' : '';[\s\S]*barEl\.className = `[\s\S]*\$\{shortClass\}/, 'updated short task bars must retain the dedicated handle layout class');
assert.match(styles, /\.tm-modal:not\(\.tm-modal--mobile\) \.tm-gantt-bar--short \.tm-gantt-bar-handle--start \{[\s\S]*left: 0;[\s\S]*transform: translateX\(-100%\);[\s\S]*\.tm-modal:not\(\.tm-modal--mobile\) \.tm-gantt-bar--short \.tm-gantt-bar-handle--end \{[\s\S]*left: 100%;[\s\S]*right: auto;/, 'short desktop handle hit areas must anchor to opposite task boundaries');
assert.match(styles, /\.tm-modal:not\(\.tm-modal--mobile\) \.tm-gantt-bar--short \.tm-gantt-bar-handle--start::before \{[\s\S]*left: calc\(100% - 4px\);[\s\S]*\.tm-modal:not\(\.tm-modal--mobile\) \.tm-gantt-bar--short \.tm-gantt-bar-handle--end::before \{[\s\S]*left: 4px;/, 'short desktop handle markers must expand symmetrically around the true date width');
