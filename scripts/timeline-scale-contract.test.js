...
assert.match(services, /const hasExplicitRowModel = Array\.isArray\(opts\.rowModel\);[\s\S]*const requestedRowModel = hasExplicitRowModel \? opts\.rowModel : __tmBuildTaskRowModel\(\);[\s\S]*if \(!hasExplicitRowModel\) state\.__tmTimelineFullRowModel = requestedRowModel;[\s\S]*__tmScheduleTimelineGroupRangeMetaWarmup\(requestedRowModel\)/, 'timeline in-place rendering must retain a full range row model while warming group metadata');
