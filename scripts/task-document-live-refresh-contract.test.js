...
assert.match(taskIncremental, /taskIds\.forEach\(\(taskId\) => __tmCustomFieldAttrValueCache\.delete\(taskId\)\)[\s\S]*readWatermarkRevision[\s\S]*await API\.getTaskById\(taskId\)[\s\S]*__tmBuildAuthoritativeTaskConfirmationCandidate\(row,[\s\S]*__tmPrepareTaskBlockIncrementalRow\(authoritativeTask,[\s\S]*__tmConfirmLocalTaskPatchWatermarkFromTask\(taskId, authoritativeTask,[\s\S]*readWatermarkRevision[\s\S]*__tmMergeLocalTaskPatchIntoTask\(nextTask\)/,
