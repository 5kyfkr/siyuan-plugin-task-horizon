...
function loadScheduledCore() {
    ...
    const context = {
        ...
        window: { siyuan: { config: { system: { kernelVersion: '3.7.3' } } } },
    };
...
async function run() {
...

    {
        ...
        completion.context.__tmAI = {
            ...
            async runAutomation() { return { title: 'Result', markdown: 'Done', sessionID: 'session-1' }; },
        };
        ...
        completion.context.__tmCallTaskHorizonKernelRpc = async (name, input) => {
            if (name === 'taskHorizonReplaceAgentSchedules') { return { available: true, data: input.events }; }
