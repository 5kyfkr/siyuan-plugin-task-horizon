...
const shellLifecycleRuntime = fs.readFileSync(path.join(root, 'src/task-horizon/main/shell/80-shell-lifecycle.js'), 'utf8');
...
assert.doesNotMatch(startupShell, /markReloadPerf|__tmTaskHorizonPerf/, 'startup must not emit performance instrumentation');
assert.equal((shellLifecycleRuntime.match(/await __tmApplyCurrentContextViewProfile\(/g) || []).length, 1, 'startup must apply the context view profile only once');
assert.match(renderStateRuntime, /const listLike = __tmIsListLikeViewMode\(value\)[\s\S]*?value === 'timeline'[\s\S]*?return viewWindowJob/, 'large timeline switches must use the shared bounded first-render window');
assert.match(sceneRuntime, /__tmTimelineFullRowModel = renderMode === 'timeline' \? __tmBuildTaskRowModel\(\) : null;/, 'timeline switches must retain a full row model for range calculations');
assert.match(sceneRuntime, /__tmTimelineProgressive[\s\S]*?__tmSliceTaskRowModelByTaskWindow/, 'timeline switches must limit only the first DOM batch');
assert.match(servicesRuntime, /const requestedRowModel = hasExplicitRowModel \? opts\.rowModel : __tmBuildTaskRowModel\(\);[\s\S]*?state\.__tmTimelineFullRowModel = requestedRowModel/, 'timeline rerenders must retain the full range model for consistent dates');
...
assert.match(kanbanRuntime, /taskStoreRevision[\s\S]*canReuseKanbanIndex[\s\S]*kanbanIndexCache\.revision === taskStoreRevision/, 'kanban base indexes must be reused only within the same task-store revision');
assert.match(kanbanRuntime, /filteredRaw === filteredRaw[\s\S]*flatTaskMap === flatTaskMap/, 'kanban base index reuse must stay scoped to the same task projection and flat task map');
assert.match(kanbanRuntime, /__tmRegisterKanbanProgressiveColumn[\s\S]*const nextLimit = progressiveColumnLimit \+[\s\S]*__tmPatchKanbanProgressiveColumn/, 'visible kanban columns must append later batches without rebuilding mounted cards');
...
assert.match(viewSwitchCommitScheduler, /requestAnimationFrame\(scheduleAfterFeedbackPaint\)/, 'view construction must wait for the feedback paint');
assert.match(viewSwitchCommitScheduler, /setTimeout\(run, 0\)/, 'view construction must start immediately after the feedback boundary');
...
assert.match(viewSwitchRuntime, /if \(next === 'whiteboard'\)[\s\S]*?__tmFitWhiteboardToVisibleCards\(\)[\s\S]*?__tmApplyWhiteboardTransform\(\)/, 'whiteboard view switches must fit the mounted canvas without triggering a second full render');
assert.doesNotMatch(viewSwitchRuntime, /next === 'whiteboard'[\s\S]*?tmWhiteboardResetView\?\.\(\)/, 'whiteboard view switches must not rebuild the entire scene just to restore the viewport');
...
assert.match(viewSwitchRuntime, /inPlace = !forceFullRender && __tmTrySwitchViewBodyInPlace\(renderedMode, next\)[\s\S]*?__tmPreserveShellDuringViewSwitchRender = true;[\s\S]*?try \{ render\(\); \} finally \{ state\.__tmPreserveShellDuringViewSwitchRender = false; \}/, 'full-render fallbacks must preserve the visible shell until the replacement is mounted');
...
assert.match(sceneRuntime, /const __tmTimelineFullRowModel = renderMode === 'timeline' \? __tmBuildTaskRowModel\(\) : null;[\s\S]*?__tmSliceTaskRowModelByTaskWindow/, 'timeline switches must keep a full range model and slice only the first DOM batch');
...
assert.match(styles, /\.tm-body--timeline \.tm-group-row \.tm-group-sticky \{[\s\S]*?box-shadow:\s*inset 0 -1px 0 var\(--tm-table-border-color\);/, 'timeline group labels must redraw the separator covered by their sticky background');
